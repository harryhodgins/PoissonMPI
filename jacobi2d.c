 ...
 */
void exchang2d_1(double x[][maxn], int nx, int row_s, int row_e,int col_s,int col_e, MPI_Comm comm,
	      int nbrleft, int nbrright,int nbrup,int nbrdown)
{
  ...
  MPI_Sendrecv(&x[col_e][row_s],lny,MPI_DOUBLE,nbrright,0,&x[col_s-1][row_s],lny,MPI_DOUBLE,nbrleft,0,comm,MPI_STATUS_IGNORE);
  // printf("(myid: %d) sent \"col\" %d with %d entries to nbr: %d\n",nbrleft, col_e, ny, nbrright); 
  // printf("(myid: %d) recvd into \"col\" %d from %d entries from nbr: %d\n",nbrright, col_s-1, ny, nbrleft);
  
  MPI_Sendrecv(&x[col_s][row_s],lny,MPI_DOUBLE,nbrleft,1,&x[col_e+1][row_s],lny,MPI_DOUBLE,nbrright,1,comm,MPI_STATUS_IGNORE);
  // printf("(myid: %d) sent \"col\" %d with %d entries to nbr: %d\n",nbrright, col_e, ny, nbrleft); 
  ...
  MPI_Sendrecv(&x[col_s][row_e],1,column_type,nbrup,2,&x[col_s][row_s-1],1,column_type,nbrdown,2,comm,MPI_STATUS_IGNORE);
  MPI_Sendrecv(&x[col_s][row_s],1,column_type,nbrdown,3,&x[col_s][row_e+1],1,column_type,nbrup,3,comm,MPI_STATUS_IGNORE);

  MPI_Type_free(&column_type);
}

void sweep2d(double a[][maxn], double f[][maxn], int nx,
	     int row_s, int row_e,int col_s,int col_e, double b[][maxn])
{
  double h;
  int i,j;
...
double griddiff2d(double a[][maxn], double b[][maxn], int nx, int row_s, int row_e,int col_s,int col_e)
{
  double sum;
  double tmp;
  int i, j;
