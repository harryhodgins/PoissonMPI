 ...
 * @brief A program which computes the solution to the discretised Poisson equation using the Jacobi method for finite difference matrix.
          Uses a 1D domain decomposition.
 * @version 1.0
 * @date 2024-04-15
 * 
 * 
...
void write_grid(char* filename,double a[][maxn],int nx,int ny,int rank,int s,int e);
void GatherGrid(double global_grid[maxn][maxn],double a[maxn][maxn],int s,int e,int nx,int ny,int myid,int nprocs,int *s_vals,int *e_vals);
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  double global_grid[maxn][maxn]; //the array which will contain the gathered grids.
  GatherGrid(global_grid,a,s,e,nx,ny,myid,nprocs,s_vals,e_vals); //gather the grids onto proc 0.
 ...
 * @param s_vals Array holding values of start indexes for each process in order.
...
void GatherGrid(double global_grid[maxn][maxn],double a[maxn][maxn],int s,int e,int nx,int ny,int myid,int nprocs,int *s_vals,int *e_vals)
{
  ...
  if(myid != 0) //if id is not root, send the columns to root
  {
    ...
    for(p = s;p<=e;p++)
    {
      ...
      MPI_Send(&a[p][1],nx,MPI_DOUBLE,0,p+myid,MPI_COMM_WORLD); //send col p to root
      //printf("sent\n");
    } 
  }
  if(myid == 0) //receive the cols
  {
    ...
    for(proc = 1;proc<nprocs;proc++)
    {
      ...
      {
        MPI_Recv(&global_grid[col][1],nx,MPI_DOUBLE,proc,proc+col,MPI_COMM_WORLD,&status);
