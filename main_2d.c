 ...
 * @brief A program which computes the solution to the discretised Poisson equation using the Jacobi method for finite difference matrix.
          Uses a 2D domain decomposition.
 * @version 1.0
 * @date 2024-04-15
 * 
 * 
...
int MPE_Decomp2d(int nrows, int ncols, int rank, int *coords,
                 int *row_s, int *row_e, int *col_s, int *col_e, int *dims);
void GatherGrid(double global_grid[maxn][maxn],double a[maxn][maxn],int row_s,int row_e,int col_s,int col_e,int nx,int ny,int myid,int nprocs,int *row_s_vals,int *row_e_vals,int *col_s_vals,int *col_e_vals);
void write_grid(char* filename,double a[][maxn],int nx,int ny,int rank,int row_s,int row_e,int col_s,int col_e);
...
if (coords[1] == dims[1] - 1) { // last column
  ...
  for(it=0; it<maxit; it++){
    ...
    exchang2d_1(a,nx,row_s,row_e,col_s,col_e,MPI_COMM_WORLD,nbrleft,nbrright,nbrup,nbrdown);
    sweep2d(a,f,nx,row_s,row_e,col_s,col_e,b);

    exchang2d_1(b,nx,row_s,row_e,col_s,col_e,MPI_COMM_WORLD,nbrleft,nbrright,nbrup,nbrdown);
  ...
  write_grid("output2d",a, nx,ny,myid,row_s,row_e,col_s,col_e); //local grids
  ...
  GatherGrid(global_grid,a,row_s,row_e,col_s,col_e,nx,ny,myid,nprocs,row_s_vals,row_e_vals,col_s_vals,col_e_vals);
  if(myid == 0)
  {
    write_grid("global_2d_31",global_grid, nx,ny,myid,1,maxn-2,1,maxn-2);
  }
  MPI_Finalize();
...
/**
 * @brief Caluclates which rows and columns each processor is responsible for. Makes use of the Cart Create method.
 * @return int 
 */
                 ...
                 int *row_s, int *row_e, int *col_s, int *col_e, int *dims)
{
    ...
    *row_s = coords[0] * rows_per_proc + ((coords[0] < row_deficit) ? coords[0] : row_deficit) + 1;
    if (coords[0] < row_deficit) rows_per_proc++;
    *row_e = *row_s + rows_per_proc - 1;
    if (*row_e > nrows || coords[0] == dims[0]-1) *row_e = nrows;
    int total_rows_assigned = *row_e - *row_s + 1; // The total number of rows assigned to this rank
    *row_e = nrows - (*row_s - 1); // Invert the start index to get the new end index
    *row_s = *row_e - total_rows_assigned + 1; 

    //Column decomposition
    cols_per_proc = ncols / dims[1];
    col_deficit = ncols % dims[1];
    *col_s = coords[1] * cols_per_proc + ((coords[1] < col_deficit) ? coords[1] : col_deficit) + 1;
    if (coords[1] < col_deficit) cols_per_proc++;
    *col_e = *col_s + cols_per_proc - 1;
    if (*col_e > ncols || coords[1] == dims[1]-1) *col_e = ncols;

    return MPI_SUCCESS;
}

/**
 * @brief A function which writes the grid belonging to each processor to file and also to stdout.
 * 
 ...
 * @param a The local grid on each process. Must have conceptually column major ordering (Mesh style).
 ...
 * @param row_s_vals Array holding values of start row indexes for each process in order.
 * @param row_e_vals Array holding values of end row indexes for each process in order.
 * @param col_s_vals Array holding values of start column indexes for each process in order.
 * @param col_e_vals Array holding values of end column indexes for each process in order.
 */
void GatherGrid(double global_grid[maxn][maxn],double a[maxn][maxn],int row_s,int row_e,int col_s,int col_e,int nx,int ny,int myid,int nprocs,int *row_s_vals,int *row_e_vals,int *col_s_vals,int *col_e_vals)
{
  int lny;
  ...
  if(myid != 0) //if id is not root, send the columns to root
  {
    ...
    for(p = col_s;p<=col_e;p++)
    {
      ...
      MPI_Send(&a[p][row_s],lny,MPI_DOUBLE,0,p+myid,MPI_COMM_WORLD); //send col p to root
      //printf("sent\n");
    } 
  }
  if(myid == 0) //receive the cols
  {
    ...
    for(proc = 1;proc<nprocs;proc++)
    {
      ...
      {
        MPI_Recv(&global_grid[col][row_s_vals[proc]],lny,MPI_DOUBLE,proc,proc+col,MPI_COMM_WORLD,&status);
